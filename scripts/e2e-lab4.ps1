...
Write-Host "1) guest GET /api/brake-wear (expect 401)"
try {
  Invoke-RestMethod -Method Get -Uri ($base + "/api/brake-wear") | Out-Null
...
$draft = Invoke-RestMethod -Method Post -Uri ($base + "/api/brake-wear/cart/items") -Headers $hUser -ContentType "application/json" -Body $addBody
...
$formed = Invoke-RestMethod -Method Put -Uri ($base + "/api/brake-wear/" + $draftId) -Headers $hUser -ContentType "application/json" -Body $updBody
Write-Host ("formed status=" + $formed.status)

Write-Host "6) creator list applications (expect array, count>=1)"
$listUser = Invoke-RestMethod -Method Get -Uri ($base + "/api/brake-wear?status=formed") -Headers $hUser
Write-Host ("creator got " + $listUser.Count + " items")

Write-Host "7) moderator list applications (expect array, includes creator app)"
$listMod = Invoke-RestMethod -Method Get -Uri ($base + "/api/brake-wear?status=formed") -Headers $hMod
Write-Host ("moderator got " + $listMod.Count + " items")

Write-Host "8) creator tries finish (expect 403)"
try {
  Invoke-RestMethod -Method Put -Uri ($base + "/api/brake-wear/" + $draftId + "/finish") -Headers $hUser | Out-Null
...
$fin = Invoke-RestMethod -Method Put -Uri ($base + "/api/brake-wear/" + $draftId + "/finish") -Headers $hMod
...
try {
  Invoke-RestMethod -Method Get -Uri ($base + "/api/brake-wear?status=formed") -Headers $hMod | Out-Null
